...
class AutoOffThresholdNumber(CoordinatorEntity, NumberEntity):
    """Number entity để điều chỉnh ngưỡng Heat Index tự động tắt quạt."""
    ...
    def __init__(self, coordinator: SmartFanCoordinator, entry: ConfigEntry) -> None:
        super().__init__(coordinator)
        self.entry = entry
        self._attr_unique_id = f"{entry.entry_id}{SUFFIX_AUTO_OFF_THRESHOLD_NUMBER}"
        self._attr_has_entity_name = True
        self._attr_translation_key = "auto_off_threshold"
        slug_name = slugify(entry.data.get("fan_name", "fan")).replace("_", "")
        self.entity_id = f"number.maic_{slug_name}_{self._attr_translation_key}"
    ...
    def native_value(self) -> float:
        """Ngưỡng hiện tại."""
        return self.coordinator.data.get("auto_off_threshold", 37.0)

    async def async_set_native_value(self, value: float) -> None:
        """Cập nhật ngưỡng mới."""
        await self.coordinator.async_update_auto_off_threshold(value)
    ...
    def extra_state_attributes(self) -> dict:
        ...
        return {
            "chỉ_số_oi_bức_hiện_tại": muggy,
            "khoảng_cách_đến_ngưỡng": round(muggy - self.native_value, 1),
        }
